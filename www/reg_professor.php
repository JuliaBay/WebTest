<?php
	...
	{		
		...
		try
		{
			...
			if(!isset($form_fields["f_ID"]))
				DBAddProfessor($f_name,$f_log,$f_dep_ID,$f_skill_ID,$f_email,$f_pass);
			else
				DBEditProfessor($f_ID,$f_name,$f_log,$f_dep_ID,$f_skill_ID,$f_email,$f_pass);			
			
			header("Location: /admin-users-professors.php");
		}
		catch(Exception $ex)
		{
			$err=$ex->getMessage();
	...
	if(isset($_GET["edit_id"]) && !isset($_POST["Go"]))
	{
		...
		try
		{
			...
			$form_fields["f_skill"]=$form_fields["Skill_ID"];
			...
			<div id="reg-form">
				<fieldset>
					<form action="" method="POST">
						...
						<select type="text" id="f_skill" name="f_skill">
							<option value="-1">--Выберите степень--</option>
								<?while($item=DBFetchSkill($res)):?>
								<option value="<?=$item["ID"]?>" <?=($form_fields["f_skill"]==$item["ID"])?"selected":""?>><?=$item["name"]?></option>
								<?endwhile;?>
						</select>
