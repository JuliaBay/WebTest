<?php
	...
	$univ_selected=isset($_POST["f_univ"]) && (int)$_POST["f_univ"]>0;
	...
	{				
		...
		if(trim($err)=="")
		{
			...
			if(!isset($form_fields["f_ID"]))
				DBAddUser($f_uni_ID,$f_name,$f_fac_ID,$f_spec_ID,$f_mail,$f_pass);
			else
				DBEditUser($f_ID,$f_uni_ID,$f_name,$f_fac_ID,$f_spec_ID,$f_mail,$f_pass);
...
<html>
	...
	<body>
		...
		<form action="" method="POST">
			...
			<input id="get_fac" name="get_fac" type="submit" value="Получить список факультетов"/>
			<script>$("#get_fac").css('display','none')</script>
			<div>Факультет:</div>
			<select id="f_fac" name="f_fac" <?if(!$univ_selected){?>disabled="disabled"<?}?>>
				<option value="-1">--Выберите факультет--</option>
				<? if($univ_selected)
						while($univ=DBFetchFaculty((int)$_POST["f_univ"])):	?>
						<option <?if($univ["ID"]==$form_fields["f_fac"]){?>selected="selected"<?}?> value="<?=$univ["ID"]?>"><?=$univ["Name"]?></option>
						<?endwhile;?>
			</select>
			<div>Кафедра:</div>
